create scheme if not exists bay_col;
...
create table if not exists bay_col.tbl_details (
	...
	name			text not null,
	description		text,
	id_tbl_master integer not null references bay_col.tbl_master(id),
	created			timestamp with time zone default current_timestamp
);
...
--INICIA LA CREACIÓN DE DATOS
	...
	INSERT INTO bay_col.tbl_details(code, name, description, id_tbl_master) 
